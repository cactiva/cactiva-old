...
export default observer(() => {
  ...
  const load = async (monacoEdRef: any, path: any) => {
    if (meta.current.unsaved) {
      if (
        !confirm(
          "Current file is unsaved, your change will be lost. Continue ?"
        )
      ) {
        return false;
      }
  ...
  return (
    <div className="cactiva-dialog-editor">
          ...
          <Button
            ...
            onClick={() => {
              (async () => {
                ...
                if (newname) {
                  const relPath =
                    _.lowerCase(newname).replace(/[^0-9a-zA-Z]/g, "") + ".ts";
                  const path = "/src/stores/" + relPath;
                  editor.status = "creating";
                  try {
                    await api.get(`store/newfile?path=${path}`);
                    meta.current.path = "./" + relPath;
                    meta.current.content = await api.get(
                      `store/readfile?path=${"./" + relPath}`
                    );
                  } catch (e) {
                    console.log(e);
                  }
                          ...
                          onSelect={() => {
                            ...
                            setTimeout(async () => {
                              ...
                              if (newname && e.name !== newname) {
                                ...
                                if (e.type === "file") {
                                  path.push(
                                    newname.replace(/[^0-9a-zA-Z]/g, "") + ".ts"
                                  );
                                } else {
                                  path.push(
                                    newname.replace(/[^0-9a-zA-Z]/g, "")
                                  );
                                }
                                ...
                                try {
                                  await api.get(
                                    `ctree/move?old=${e.relativePath.replace(
                                      "./",
                                      "./src/stores/"
                                    )}&new=${path
                                      .join("/")
                        ...
                        <Menu.Item
                          ...
                          onSelect={() => {
                            ...
                            setTimeout(async () => {
                              if (
                                confirm(
                                  `Are you sure want to delete ${e.relativePath}?`
                                )
                              ) {
                                editor.status = "deleting";
                                try {
                                  await api.get(
                                    `ctree/delete?path=${e.relativePath.replace(
                                      "./",
                                      "./src/stores/"
                                    )}`
                                  );
                    ...
                    return (
                      <>
                        <div
                          ...
                          onClick={() => {
                            load(monacoEdRef, e.relativePath);
                          }}
                        >
      ...
      >
        ...
        >
          <Text style={{ padding: "0px 5px", fontSize: "10px" }}>
            {meta.current.path}
          </Text>
          <IconButton
            icon="cross"
            appearance="minimal"
            onClick={() => {
              editor.modals.store = false;
            }}
            style={{ width: 25, height: 25, padding: 2, boxShadow: "0" }}
          />
        </div>
        {meta.list.length > 0 && (
          <MonacoEditor
